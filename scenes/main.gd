...
@onready var create_new_project:  Button                = %CreateNewProjectButton

@onready var canvas_container:    PanelContainer        = %CanvasContainer
@onready var canvas:              WidgetCanvas          = $SafeAreaContainer/RootLayout/WorkArea/Panel/CanvasContainer/Canvas
...
func _connect_signals() -> void:
    ...
    # ── Centre button ─────────────────────────────────────────────────────────
    create_new_project.pressed.connect(
        func() -> void: IntentBus.new_project_requested.emit()
    )
...
func _on_file_dialog_selected(path: String) -> void:
    match file_dialog.file_mode:
        FileDialog.FILE_MODE_SAVE_FILE:
            ...
            # Path is passed separately; wire ProjectManager to handle
            # save_project_as_requested and use this path as needed.
            ProjectManager.save_project(canvas, path)
        FileDialog.FILE_MODE_OPEN_FILE:
            IntentBus.load_project_requested.emit(path)

# ── Page Navigation ───────────────────────────────────────────────────────

## Responds to a confirmed page change event from the EventBus.
## Clears the current canvas state and loads the incoming page's widget layout.
...
func _on_project_opened(project_data: ProjectData) -> void:
    ...
    var initial_page := project_data.get_default_page()
    if initial_page != null:
        canvas.load_page(initial_page)  # Direct call — bypasses dirty check on open
